...
uint8_t BMP280::bmp280Read8(uint8_t reg)
{
  ...
  while(!Wire.available()){
	  if ((millis() - a ) > 500){
		  Serial.print("Wire not available in 500ms in BMP280 read8");
	  }
  }
  return Wire.read();
}
...
uint16_t BMP280::bmp280Read16(uint8_t reg)
{
  ...
  while(Wire.available()<2){
	  if ((millis() - a ) > 500){
		  Serial.print("Wire not available in 500ms in BMP280 read16");
	  }
  }
  msb = Wire.read();
  lsb = Wire.read();
...
uint32_t BMP280::bmp280Read24(uint8_t reg)
{
  ...
  {
	if ((millis() - a ) > 500){
		  Serial.print("Wire stuck in available for 500ms in BMP280 read24");
	}
    tempData[i] = Wire.read();
    data <<= 8;
    data |= tempData[i];

//Serial.println(tempData[i],BIN);Serial.println(data,BIN);Serial.println("-");
    i++;
  }
...
//uint8_t BMP280::isAvailable()
//{
//    uint8_t status =  bmp280Read8(0xF3);
////    Serial.print("Checking availablility: "); Serial.println(status,BIN);
//    uint8_t MeasuringStatusMask = 0x1;
//    uint8_t updatingStatusMask = 0x8;
//    uint8_t mask;
//	if((status & MeasuringStatusMask ) || (status & updatingStatusMask ))
//	{
